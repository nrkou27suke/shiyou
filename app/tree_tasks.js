...
@media (max-width:560px){
  ...
  /* ⋯ は常に右端・画面内に固定 */
  .tt-more{ flex:0 0 auto; margin-left:auto; }
  /* 省スペースのため、ツリー行ではタグとメモ点を隠す（⋯の中で編集可・締切順では表示） */
  .tt-row .tt-tags, .tt-row .tt-memo-dot{ display:none; }
  /* 1行ぶん背が高くなるので、枝の終点を行の中心に合わせ直す */
  .tt-children > .tt-node::after{ top:20px; }
  .tt-children > .tt-node:last-child::before{ height:20px; }
  /* 引き出しは画面幅に合わせて余裕を持たせる */
